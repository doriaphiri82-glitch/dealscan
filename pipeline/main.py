...
def _configured_ids():
    registered={c.get("county_id") for c in list_counties()}
    return [cid for cid in DEFAULT_COUNTIES if cid in registered]

def run_all(mode='publish',only=''):
    ensure_national_counties(); ids=[only] if only else _configured_ids()
...
def cmd_run(args):init_db(); run_all(mode='etl-only' if args.etl_only else 'publish',only=args.county or '')
...
def cmd_coverage():
    ensure_national_counties(); s=county_summary(); total=s['total'] or 1; covered=sum(v for k,v in s['by_coverage_status'].items() if k in ('tier_4','tier_5')); verified=sum(v for k,v in s['by_coverage_status'].items() if k in ('tier_3','tier_4','tier_5')); discovered=sum(v for k,v in s['by_coverage_status'].items() if k in ('tier_1','tier_3','tier_4','tier_5'))
