...
def cmd_discover(args):
    ...
    result=discover_and_register(args.discover_national)
...
def cmd_run_national(args):
    ...
    init_db(); result=run_national_batch(args.run_national,args.max_records,'etl-only' if args.etl_only else 'publish')
...
def cmd_coverage():
    ...
    verified=sum(v for k,v in s['by_coverage_status'].items() if k in ('tier_3','tier_4','tier_5'))
    discovered=sum(v for k,v in s['by_coverage_status'].items() if k in ('tier_1','tier_3','tier_4','tier_5'))
    print(f"National counties: {s['total']}")
    print(f"Sources discovered: {discovered} ({discovered/total:.1%})")
    print(f"Sources ETL-verified: {verified} ({verified/total:.1%})")
...
def main():
    ...
    parser.add_argument('--discover-national',type=int,metavar='N',help='Discover up to N new public ArcGIS sources')
