...
def discover_services(rest_root: str) -> Optional[Dict[str, Any]]:
    url=rest_root.rstrip("/")+"/arcgis/rest/services?f=json"; r=fetch(url,ttl=24*3600,as_json=True,respect_robots=False)
    return r.body if r.ok and isinstance(r.body,dict) else None
...
def layer_fields(layer_url: str) -> Optional[List[str]]:
    r=fetch(f"{layer_url}?f=json",ttl=24*3600,as_json=True,respect_robots=False)
    if not r.ok or not isinstance(r.body,dict): return None
    return [f.get("name") for f in r.body.get("fields") or [] if f.get("name")]
...
def query_layer(layer_url: str, where: str, out_fields: List[str], max_records: int=5000, page_size: int=1000) -> Iterator[Dict[str, Any]]:
    """Query ArcGIS with explicit pagination; continue while exceededTransferLimit is true."""
    offset=0; fields=",".join(out_fields) if out_fields else "*"; target=max(1,int(max_records)); size=max(1,min(int(page_size),target))
    while offset < target:
        count=min(size,target-offset)
        payload={"where":where,"outFields":fields,"returnGeometry":"false","f":"json","resultOffset":offset,"resultRecordCount":count}
        ...
        got=len(feats)
        more=bool(r.body.get("exceededTransferLimit"))
        if not more: return
...
def map_attributes(attrs: Dict[str, Any], field_map: Dict[str, Any], county_id: str, defaults: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out["lot_size_acres"]=_to_float(out.get("lot_size_acres")); out["assessed_value"]=_to_float(out.get("assessed_value")); out["market_value"]=_to_float(out.get("market_value")); out["tax_amount"]=_to_float(out.get("tax_amount")); out["improvement_value"]=_to_float(out.get("improvement_value")); out["tax_delinquent_years"]=_to_int(out.get("tax_delinquent_years")); out["year_acquired"]=_to_int(out.get("year_acquired")); out["latitude"]=_to_float(out.get("latitude")); out["longitude"]=_to_float(out.get("longitude")); out["county_id"]=county_id
    return out
...
def is_vacant_residential(prop: Dict[str, Any], county_id: str) -> bool:
    lu=str(prop.get("land_use") or "").strip().lower(); zoning=str(prop.get("zoning") or "").strip().lower(); code=str(prop.get("use_code") or prop.get("land_use") or "").strip().upper(); imp=prop.get("has_improvements"); improvement_value=prop.get("improvement_value")
    has_imp=imp is True or imp in (1,"1","Y","YES","Yes","true","True"); no_imp=imp is False or imp==0 or imp in ("0","N","NO","No","NONE","false","False")
