...
class SupabaseDatabase:
    ...
    def record_ingestion_run(self, county_id: str, status: str, counts: Dict[str, Any], error: str = "", source_url: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> int:
        ...
        payload = {"county_id": county_id, "run_type": "scheduled" if os.getenv("GITHUB_ACTIONS") else "manual", "status": status_map.get(status, "failed"), "source_url": source_url, "records_seen": int(counts.get("discovered", 0) or 0), "records_normalized": int(counts.get("normalized", 0) or 0), "records_persisted": int(counts.get("stored", 0) or 0), "records_rejected": int(counts.get("rejected", 0) or 0), "error_message": error or None, "metadata": metadata or {}}
        rows = self._request("POST", "ingestion_runs", headers={**self.headers, "Prefer": "return=representation"}, json=payload).json()
        if not rows: raise RuntimeError("Supabase ingestion run insert returned no row")
        return int(rows[0]["id"])

    def update_ingestion_run(self, run_id: int, county_id: str, status: str, counts: Dict[str, Any], error: str = "", metadata: Optional[Dict[str, Any]] = None) -> int:
        status_map = {"ok": "completed", "degraded": "partial", "error": "failed", "skipped": "partial"}
        payload = {"status": status_map.get(status, "failed"), "records_seen": int(counts.get("discovered", 0) or 0), "records_normalized": int(counts.get("normalized", 0) or 0), "records_persisted": int(counts.get("stored", 0) or 0), "records_rejected": int(counts.get("rejected", 0) or 0), "error_message": error or None, "metadata": metadata or {}}
        rows = self._request("PATCH", "ingestion_runs", params={"id": f"eq.{int(run_id)}", "county_id": f"eq.{county_id}"}, headers={**self.headers, "Prefer": "return=representation"}, json=payload).json()
        if not rows: raise RuntimeError("Supabase ingestion run update matched no row")
        return int(rows[0]["id"])

    def ensure_active_ingestion_run(self, county_id: str, source_url: Optional[str] = None) -> Optional[int]:
        if os.getenv("DEALSCAN_DB_BACKEND", "sqlite").strip().lower() != "supabase": return None
        active = os.getenv("DEALSCAN_ACTIVE_AUDIT_RUN_ID")
        if active:
            try: return int(active)
            except ValueError: os.environ.pop("DEALSCAN_ACTIVE_AUDIT_RUN_ID", None)
        run_id = self.record_ingestion_run(county_id, "degraded", {}, source_url=source_url, metadata={"bootstrap": True})
        os.environ["DEALSCAN_ACTIVE_AUDIT_RUN_ID"] = str(run_id)
        return run_id
    ...
    def record_ingestion_records(self, run_id: int, county_id: str, records: List[Dict[str, Any]]) -> int:
        ...
        for start in range(0, len(records), 250):
            ...
            for item in records[start:start + 250]:
                raw = item.get("raw_payload"); normalized = item.get("normalized_payload") or {}
                batch.append({"run_id": int(run_id), "county_id": county_id, "source_record_id": str(item.get("source_record_id") or normalized.get("apn") or "")[:500] or None, "source_url": item.get("source_url"), "raw_payload": raw if isinstance(raw, dict) else {"value": str(raw)[:5000]} if raw is not None else {}, "normalized_payload": normalized if isinstance(normalized, dict) else {}, "property_id": int(item["property_id"]) if item.get("property_id") is not None else None, "status": str(item.get("status") or "normalized")[:100], "rejection_reason": str(item.get("rejection_reason") or "")[:500] or None})
            self._request("POST", "ingestion_records", json=batch)
            inserted += len(batch)
        return inserted

    def upsert_county(self, county: Dict[str, Any]) -> None:
        county_id = str(county.get("county_id") or "").strip(); county_name = str(county.get("county_name") or "").strip()
        if not county_id or not county_name: raise ValueError("county metadata requires county_id and county_name")
        ...
        payload = {k: county.get(k) for k in known if county.get(k) is not None}; payload["county_id"] = county_id; payload["county_name"] = county_name; payload["field_mapping"] = county.get("field_mapping") or {}; payload["extra"] = {k: v for k, v in county.items() if k not in known}
    ...
    def ensure_county(self, county: Dict[str, Any]) -> None:
        ...
        if not county_id: raise ValueError("county metadata requires county_id")
        rows = self._request("GET", "counties", params={"county_id": f"eq.{county_id}", "select": "county_id"}).json()
        if not rows: self.upsert_county(county)

    def save_property(self, data: Dict[str, Any]) -> int:
        county = data.get("_county_metadata")
        if county: self.upsert_county(county)
        else: self.ensure_county({"county_id": data["county_id"], "county_name": data.get("county_name") or data["county_id"]})
        ...
        try:
            ...
            if run_id:
                self.record_ingestion_records(run_id, data["county_id"], [{"source_record_id": data.get("_source_record_id"), "source_url": data.get("source_url"), "raw_payload": data.get("_raw_payload") or {}, "normalized_payload": {k:v for k,v in data.items() if not k.startswith("_")}, "property_id": property_id, "status": "persisted"}])
        except Exception as exc:
            print(f"WARNING: ingestion record audit failed: {str(exc)[:300]}")
        return property_id

    def save_deal(self, data: Dict[str, Any]) -> int:
        pid = int(data["property_id"]); existing = self._request("GET", "deals", params={"property_id": f"eq.{pid}", "select": "id", "order": "id.desc", "limit": "1"}).json()
        ...
        payload["status"] = data.get("status") or "discovered"
        payload["verification_status"] = data.get("verification_status") or "pending_review"
        if existing:
            did = int(existing[0]["id"]); self._request("PATCH", "deals", params={"id": f"eq.{did}"}, json=payload); return did
        payload["property_id"] = pid; rows = self._request("POST", "deals", headers={**self.headers, "Prefer": "return=representation"}, json=payload).json()
        if not rows: raise RuntimeError("Supabase deal insert returned no row")
        return int(rows[0]["id"])

    def save_comps(self, deal_id: int, comps: List[dict]) -> int:
        self._request("DELETE", "comps", params={"deal_id": f"eq.{int(deal_id)}"}); valid=[]
    ...
    def get_deal_comps(self, deal_id: int) -> List[dict]: return self._request("GET", "comps", params={"deal_id": f"eq.{int(deal_id)}", "select": "address,sale_price,sale_date,distance_miles,lot_size_acres,price_per_acre", "order": "distance_miles.asc"}).json()
    ...
    def add_waitlist_entry(self,email:str,source:str="unknown")->None: self._request("POST","waitlist",headers={**self.headers,"Prefer":"resolution=ignore-duplicates"},json={"email":email,"source":source})
