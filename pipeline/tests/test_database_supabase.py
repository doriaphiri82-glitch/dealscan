...
class FakeResponse:
    def __init__(self, payload=None, status_code=200, text=""):
        ...
        self.headers = {}; self.status_code = status_code; self.text = text; self.ok = status_code < 400
...
def test_property_and_deal_round_trip_payloads(monkeypatch):
    ...
    monkeypatch.setattr(db.session, "request", fake_request)
...
def test_source_validation_never_promotes_a_deal_to_verified(monkeypatch):
    ...
    assert did == 88
    assert captured["payload"]["verification_status"] == "pending_review"
...
def test_errors_include_status_but_not_sensitive_response_body(monkeypatch):
    db=SupabaseDatabase("https://example.supabase.co","service-key")
    monkeypatch.setattr(db.session, "request", lambda *a,**k: FakeResponse(status_code=500,text="database unavailable"))
    monkeypatch.setattr("database_supabase.time.sleep", lambda _: None)
    with pytest.raises(RuntimeError, match="HTTP 500") as exc: db.get_top_deals()
    assert "database unavailable" not in str(exc.value)
