...
def discover_and_register(limit: int = 25) -> Dict[str, Any]:
    ...
    candidates = [c for c in list_counties() if c.get("coverage_status", "tier_0") != "tier_5" and not c.get("arcgis_layer_url") and not c.get("parcel_source_url")]
    results=[]; found=0
    for county in candidates[:_limit(limit)]:
        ...
        try:
            ...
            update_county(cid,data_source_type="arcgis",gis_url=cfg.get("arcgis_root"),parcel_source_url=cfg.get("arcgis_layer_url"),source_vendor="esri",scraper_type="arcgis",verification_status="discovered_not_verified",coverage_status="tier_1",field_mapping=fields,data_freshness=str(freshness) if freshness is not None else None,notes=f"Public ArcGIS source discovered; ETL verification pending; source quality={quality}",extra={"arcgis_layer_url":cfg.get("arcgis_layer_url"),"discovery_source":cfg.get("discovery_source"),"discovery_score":cfg.get("discovery_score"),"field_count":len(fields),"source_quality":quality,"source_quality_score":cfg.get("source_quality_score",0),"useful_field_count":cfg.get("useful_field_count",0),"missing_useful_fields":cfg.get("missing_useful_fields",[]),"source_last_modified":freshness})
...
def run_national_batch(limit:int=10,max_records:int=5000,mode:str="publish")->Dict[str,Any]:
    """Run every eligible county, not just the pilot counties; prioritize counties never successfully run."""
    ensure_national_counties()
    candidates=[c for c in list_counties() if c.get("coverage_status")!="tier_5" and (c.get("arcgis_layer_url") or c.get("parcel_source_url") or c.get("arcgis_root"))]
    ...
    for county in candidates[:_limit(limit,10)]:
        try:
            result=run_county(county["county_id"],mode=mode,max_records=max(1,min(int(max_records),10000)))
