...
def save_property(data: dict) -> int:
    ...
    cur.execute('SELECT id FROM properties WHERE apn=? AND county_id=?',(data['apn'],data['county_id']))
...
# Keep the public database API unchanged for callers. Production selection is explicit,
# so local tests and developer environments remain SQLite unless configured otherwise.
if _USE_SUPABASE:
