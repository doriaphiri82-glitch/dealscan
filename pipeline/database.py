"""DealScan database management with SQLite local fallback and Supabase production mode."""
import os, sqlite3
from typing import List, Optional
from config.settings import DATABASE_PATH
_USE_SUPABASE = os.getenv("DEALSCAN_DB_BACKEND", "sqlite").strip().lower() == "supabase"
...
def init_db():
    ...
    CREATE INDEX IF NOT EXISTS idx_properties_county ON properties(county_id); CREATE INDEX IF NOT EXISTS idx_deals_status_score ON deals(status, deal_score DESC); CREATE INDEX IF NOT EXISTS idx_deals_property ON deals(property_id); CREATE INDEX IF NOT EXISTS idx_deals_county_score ON deals(property_id, deal_score DESC);
    ''')
    for column, definition in {'source_url':'TEXT','source_vendor':'TEXT','source_quality':'TEXT','verification_status':'TEXT','data_freshness':'TEXT','valuation_basis':'TEXT','valuation_confidence':'REAL'}.items():
...
if _USE_SUPABASE:
    ...
    get_deal_comps = _supabase.get_deal_comps
    get_top_deals = _supabase.get_top_deals
    get_subscribers = _supabase.get_subscribers
    add_waitlist_entry = _supabase.add_waitlist_entry
