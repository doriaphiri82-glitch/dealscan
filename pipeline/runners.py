...
def fetch_parcels(cfg:Dict[str,Any],county_id:str,max_records:int=5000):
    ...
    if adapter:
        ...
        if result.errors: raise RuntimeError(f"source error after {len(normalized)} normalized records: {'; '.join(result.errors[:3])}")
        result.metadata["resolved_layer_url"]=cfg.get("arcgis_layer_url"); return normalized,result
...
class RunMetrics:
    ...
    def __init__(self,county_id:str)->None:self.county_id=county_id; self.discovered=self.downloaded=self.parsed=self.normalized=self.rejected=self.stored=self.scored=self.qualified=self.published=0; self.rejection_reasons={}; self.errors=[]
...
def run(county_id:str,mode:str="publish",max_records:int=5000,dry_run:bool=False,offline:bool=False)->Dict[str,Any]:
    ...
    try:
        ...
        if scrape_result:m.discovered=scrape_result.discovered;m.downloaded=scrape_result.downloaded;m.parsed=scrape_result.parsed;m.normalized=scrape_result.normalized;m.rejected=scrape_result.rejected;m.rejection_reasons.update(scrape_result.rejection_reasons);m.errors.extend(scrape_result.errors)
        ...
        if m.normalized and len(vacant)<m.normalized:m.rejected+=m.normalized-len(vacant);m.rejection_reasons['not_vacant_residential']=m.normalized-len(vacant)
        for prop in vacant:
