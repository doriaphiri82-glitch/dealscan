...
def _save_registry(reg):
    os.makedirs(REGISTRY_DIR,exist_ok=True); tmp=REGISTRY_PATH+".tmp"
...
def mark_county_run(county_id:str,*,record_count:int,qualified_count:int=0,published_count:int=0,persisted_count:int=0,status:str="ok",error:str=""):
    """Promote coverage only when ETL evidence supports the tier."""
    entry=get_county(county_id)
    if not entry:return None
    now=datetime.now(timezone.utc).isoformat(); fields={"last_record_count":int(record_count)}
    ...
    elif status in ("ok","degraded") and record_count>0:
        fields.update({"last_successful_run":now,"data_freshness":now,"verification_status":"source_verified","coverage_status":"tier_3"})
    elif record_count==0:
        # A reachable source with zero rows is not ETL coverage. Keep the
        # previous source tier but make the failed/empty run explicit.
        fields["verification_status"]="source_verified" if entry.get("data_source_type") else "discovered_not_verified"
        fields["coverage_status"]=entry.get("coverage_status") or "tier_0"
    else:
        fields["verification_status"]="discovered_not_verified" if entry.get("data_source_type") else "not_started"; fields["coverage_status"]=entry.get("coverage_status") or "tier_0"
    note=f"Last ETL: records={record_count}, persisted={persisted_count}, qualified={qualified_count}, published={published_count}, status={status}."; note+=f" Error: {error[:300]}" if error else ""
