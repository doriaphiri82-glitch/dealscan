...
Checks the actual ArcGIS layer schema and a small sample. Validation state is
stored separately from ETL coverage so a healthy source is never mistaken for
published parcel data.
"""
...
def _resolve_layer(cfg: Dict[str, Any]) -> str:
    if cfg.get("arcgis_layer_url"):
        return str(cfg["arcgis_layer_url"]).rstrip("/")
...
def validate_county_live(county: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not layer:
        result={"county_id":cid,"status":"unreachable","errors":["no resolvable ArcGIS parcel layer"],"warnings":[]}
        mark_county_validation(cid,status="unreachable",errors=result["errors"])
        return result
    try:
        ...
        error=str(exc)[:300]
        mark_county_validation(cid,status="unreachable",errors=[error],source_fields_checked=False)
...
def validate_live_batch(limit:int=25, include_validated:bool=False) -> Dict[str,Any]:
    """Validate a bounded slice of every county with a discovered/configured source.

    Counties are selected by oldest validation timestamp so repeated workflow
    runs eventually cycle through the complete national universe.
    """
    ...
    if not include_validated:
        unvalidated=[c for c in counties if not c.get("last_validated_at")]
        validated=[c for c in counties if c.get("last_validated_at")]
        unvalidated.sort(key=lambda c:(c.get("state", ""),c.get("county_name", "")))
        validated.sort(key=lambda c:c.get("last_validated_at") or "")
        counties=unvalidated+validated
    batch=counties[:max(1,min(int(limit),100))]
    results=[validate_county_live(c) for c in batch]
