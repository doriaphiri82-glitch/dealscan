...
const inventoryCount=(value:unknown):number=>{
  if(typeof value!=='number'||!Number.isSafeInteger(value)||value<0)throw new Error('Invalid inventory count')
  return value
}
...
export function countyHealth(snapshot: unknown, now = Date.now()) {
  ...
  if (typeof stored.county_id !== 'string'||!/^[a-zA-Z0-9_-]{1,150}$/.test(stored.county_id)) throw new Error('Invalid county snapshot')
  const countyId=stored.county_id
  const published=inventoryCount(metrics.verified_total),records=inventoryCount(metrics.stored_total)
  if(published>records)throw new Error('Verified inventory exceeds stored inventory')
  ...
  const ingested = records>0 && county.ingestion_status === 'ingested' && numeric(county.persisted_count) > 0
  const status = failed || unavailable ? 'failed' : partial || (!ready && records>0) ? 'degraded'
  ...
  return {
    county_id: countyId, county_name: text(county.county_name), state: text(county.state),
