      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-10 md:gap-12 mb-12">
          <div>
            ...
            {['Pricing', 'Published opportunities', 'Capabilities', 'Workflow'].map((x, i) => <a key={x} href={['#pricing','#verified-opportunities','#capabilities','#how-it-works'][i]} className="block text-[13px] text-[#64716a] hover:text-[#176b45] py-1 transition-colors">{x}</a>)}
          </div>
          <div>
            <h4 className="font-mono text-[11px] font-semibold uppercase tracking-[0.08em] text-[#8a958f] mb-3.5">Resources</h4>
            {['Deal analysis', 'FAQ', 'Early access'].map((x, i) => <a key={x} href={['#verified-opportunities','#faq','#early-access'][i]} className="block text-[13px] text-[#64716a] hover:text-[#176b45] py-1 transition-colors">{x}</a>)}
          </div>
          <div>
            <h4 className="font-mono text-[11px] font-semibold uppercase tracking-[0.08em] text-[#8a958f] mb-3.5">Legal</h4>
            <a href="/privacy" className="mb-2 block text-sm text-[#64716a]">Privacy & data handling</a>
        ...
        <div id="disclaimer" className="border-t border-[#e5ebe7] pt-7">
          <p className="text-xs text-[#8a958f] leading-[1.7] max-w-[700px]">DealScan provides informational analysis and screening tools only. Data may be incomplete or inaccurate and should be independently verified with county and authoritative sources. DealScore and valuation estimates are not investment advice, appraisals, or guarantees of profitability. Screening is not a substitute for due diligence. Waitlist emails are used only to contact you about early access. Saved parcels and research remain in this browser profile, not a cloud-synced account. Paid subscriptions and automated email alerts are not enabled. © 2026 DealScan. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
