...
export default function DealsPage() {
  ...
  useEffect(() => {
    fetch('/api/deals?limit=50', { cache: 'no-store' })
      .then((res) => res.json())
      .then((json: DealsResponse) => setData(json))
      .catch(() => setData({ deals: [] }))
    ...
    <main className="min-h-screen bg-[#f6f8f7] text-[#13221c]">
      <header className="sticky top-0 z-20 border-b border-black/5 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-4">
          ...
          <a href="/" className="rounded-full border border-black/10 px-4 py-2 text-sm font-semibold transition hover:bg-black hover:text-white">Back home</a>
        </div>
      </header>

      <section className="mx-auto max-w-7xl px-6 pb-16 pt-12">
        <div className="mb-8 max-w-3xl">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-emerald-700">Live deal intelligence</p>
        ...
        <div className="mb-8 grid gap-3 rounded-3xl border border-black/5 bg-white p-4 shadow-[0_20px_70px_rgba(0,0,0,0.06)] md:grid-cols-[1.5fr_1fr_1fr_1fr]">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search address, APN, county…" className="rounded-2xl border border-black/10 bg-[#f8faf9] px-4 py-3 outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10" />
          <select value={county} onChange={(e) => setCounty(e.target.value)} className="rounded-2xl border border-black/10 bg-[#f8faf9] px-4 py-3 outline-none">
            <option value="all">All counties</option>
            {counties.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <select value={String(minScore)} onChange={(e) => setMinScore(Number(e.target.value))} className="rounded-2xl border border-black/10 bg-[#f8faf9] px-4 py-3 outline-none">
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value as typeof sort)} className="rounded-2xl border border-black/10 bg-[#f8faf9] px-4 py-3 outline-none">
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-dashed border-black/15 bg-white p-12 text-center">
            ...
            <p className="mx-auto mt-2 max-w-xl text-black/55">The discovery pipeline is continuously expanding its source registry. Try clearing the filters or check back after the next verified batch.</p>
          </div>
        ) : (
            ...
            {filtered.map((deal, index) => (
              <article key={`${deal.apn}-${index}`} className="group rounded-3xl border border-black/5 bg-white p-6 shadow-[0_15px_50px_rgba(0,0,0,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(0,0,0,0.1)]">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-black/40">{deal.county_id ?? 'Unknown county'}</p>
                ...
                <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold">
                  ...
                  {deal.valuation_basis && <span className="rounded-full bg-black/[0.04] px-3 py-1.5">{deal.valuation_basis.replaceAll('_', ' ')}</span>}
                  {deal.verification_status && <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-emerald-700">{deal.verification_status.replaceAll('_', ' ')}</span>}
                </div>

                <div className="mt-5 flex items-center justify-between border-t border-black/5 pt-4 text-xs text-black/40">
                  <span>APN {deal.apn ?? '—'}</span>
